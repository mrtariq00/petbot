...
let bottlesimg = document.getElementById("bottleimg");

function Bottlesimg() {
  bottlesimg.innerHTML = ``;
    products.filter((p) => p.cat === "Bottle").map((p) => {
        ...
        <a href="/pages/product.html?name=${p.name}&img=${p.img}&color=${p.color}&qty=${p.qty}&description=${p.description}&price=${p.price}" class="text-decoration-none">
             <div class="card mb-3" style="width: 350px;">
                 <img src="${p.img}" class="card-img-top h-100 w-100" alt="...">
            </div>
            </a>`;
    });
}
...
let productimg = document.getElementById("jarsimg");
